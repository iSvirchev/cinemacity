...
def broadcast_new_movies(diff_set, broadcast_list, cin_name):
    ...
    logger.info("Movies to broadcast: %s", str(diff_set))
    ...
    logger.info("Broadcast failed for %d users!", len(failed_list))
    if len(failed_list) == 0:
        ...
        for failed_user in failed_list:
            ...
            logger.warning("Broadcast failed for user: %s, status_code: %s, reason: %s" % (
            ...
            if failed_status_code == ErrorCodes.RECEIVER_NOT_SUBSCRIBED:
                ...
                logger.error("Problem with broadcasting to '%s' check status_message: '%s'" % (
...
for cinema_id, cinema in broadcast_movies_result.items():
    ...
    if broadcast_movies is not None:
        ...
        if users_to_broadcast:  # Only broadcast to the subscribed users (if any)
            logger.info("Cinema '%s' - starting a broadcast...", cinema_name)
            broadcast_new_movies(broadcast_movies, users_to_broadcast, cinema_id, cinema_name)
        else:
            logger.info("Cinema '%s' - no users subscribed to that cinema.", cinema_name)
        logger.info("Cinema '%s' - broadcast has finished. Resetting broadcast_movies for that cinema...",
                    cinema_name)
        db.reset_broadcast_movies(cinema_id)
        logger.info("Cinema '%s' - cinemas.broadcast_movies has been reset!", cinema_name)
    else:
        logger.info("Cinema '%s' - nothing to broadcast!", cinema_name)
